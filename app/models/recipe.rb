...
class Recipe < ActiveRecord::Base
    ...
    def self.find_in_api(calories, budget, time, cuisine)
        ...
        meal_types.each_with_index do |elem, idx|
        	...
        	# 	cuisine_param = ''
        	# else
        	# 	cuisine_param = '&cuisineType=' + cuisine
        	# end
            uri = URI(base_uri + '?q=' + elem + '&app_id=' + ENV["APP_ID"] + '&app_key=' +  ENV["APP_KEY"] + '&from=0&to=1&calories=' + ((calories / 3) - 100).to_s + '-' + ((calories / 3) + 100).to_s + '&time=' + 1.to_s + '-' + (time / 3).to_s + cuisine)
            byebug
